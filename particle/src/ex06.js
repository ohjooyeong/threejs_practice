...
export default function example() {
    ...
    const spherePostionArray = sphereGeometry.attributes.position.array;
    const randomPositionArray = [];

    for (let i = 0; i < spherePostionArray.length; i++) {
        randomPositionArray.push((Math.random() - 0.5) * 10);
    }
    console.log(spherePostionArray, randomPositionArray);

    // 여러개의 Plane Mesh 생성
    const imagePanels = [];
    let imagePanel;
    for (let i = 0; i < spherePostionArray.length; i += 3) {
        imagePanel = new ImagePanel({
            textureLoader,
            scene,
            geometry: planeGeometry,
            imageSrc: `/images/0${Math.ceil(Math.random() * 5)}.jpg`,
            x: spherePostionArray[i],
            y: spherePostionArray[i + 1],
            z: spherePostionArray[i + 2],
        });
