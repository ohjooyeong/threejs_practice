...
export default function example() {
  ...
  function draw() {
    ...
    for (let i = 0; i < positionArray.length; i += 3) {
      positionArray[i] += Math.sin(time + randomArray[i] * 50) * 0.002;
      positionArray[i + 1] += Math.sin(time + randomArray[i + 1] * 50) * 0.002;
      positionArray[i + 2] += Math.sin(time + randomArray[i + 2] * 50) * 0.002;
    }
